...
def generate_drawing(job, output_dir):
    ...
    max_front_w = 245
    max_side_w = 140
    scale = min(max_h / height, max_front_w / width, max_side_w / depth)
    fw, fh, sw = width * scale, height * scale, depth * scale
    x_front, y_base = 100, 167
    x_side = max(455, x_front + fw + 100)
    ...
    _view_label(pdf, "TOP VIEW", x_front, top_y + top_h + 8, fw)

    pdf.setFont("Helvetica-Bold", 10)
    pdf.drawString(455, 132, "CABINET CONFIGURATION")
    pdf.setFont("Helvetica", 9)
    details = [
    ...
    for idx, detail in enumerate(details):
        pdf.drawString(455, 116 - idx * 14, detail)
